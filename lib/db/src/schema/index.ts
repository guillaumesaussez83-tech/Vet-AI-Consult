// lib/db/src/schema/index.ts
// AUTO-GENERATED — audit Phase 0 — 2026-05-07
// Exporte explicitement les 47 tables Drizzle du schema VetoAI
// NOTE: l'ancien export './schema' (fichier inexistant) a ete supprime

export * from "./actes-consultations";
export * from "./actes";
export * from "./ai-outputs";
export * from "./ai-usage-logs";
export * from "./alertes-stock";
export * from "./anesthesie-protocoles";
export * from "./assistants";
export * from "./audit-logs";
export * from "./bons-livraison";
export * from "./client-letters";
export * from "./clinics";
export * from "./commandes-centravet";
export * from "./communications";
export * from "./consultation-attachments";
export * from "./consultation-patients";
export * from "./consultations";
export * from "./conversations";
export * from "./cremation-partners";
export * from "./exceptions-planning";
export * from "./factures";
export * from "./fournisseurs";
export * from "./ia-audit-log";
export * from "./lignes-commande";
export * from "./medications-anmv";
export * from "./messages";
export * from "./mouvements-stock";
export * from "./ordonnances";
export * from "./owners";
export * from "./parametres-clinique";
export * from "./patients";
export * from "./planning-semaine-type";
export * from "./portail-tokens";
export * from "./rappels";
export * from "./recurring-appointments";
export * from "./registre-stupefiants";
export * from "./relances";
export * from "./rendez-vous";
export * from "./rotations-weekend";
export * from "./sms-log";
export * from "./stock-lots";
export * from "./stock-medicaments";
export * from "./stock";
export * from "./user-permissions";
export * from "./vaccinations";
export * from "./ventes";
export * from "./veterinaires";
export * from "./weight-history";
export * from "./analytics-snapshots";
export * from "./group-reports";

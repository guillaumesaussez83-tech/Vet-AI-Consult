...
export function AppLayout({ children }: { children: ReactNode }) {
  ...
  const navigation = [
    ...
    { name: "StupÃÂÃÂ©fiants", href: "/stupefiants", icon: FlaskConical },
    { name: "Actes & Produits", href: "/actes", icon: Syringe },
    { name: "ParamÃÂÃÂ¨tres", href: "/parametres", icon: Settings },
  { name: "Ventes", href: "/ventes", icon: ShoppingCart },
  { name: "ÃÂÃÂquipe", href: "/equipe", icon: UserCog },
  { name: "Permissions", href: "/admin/permissions", icon: Shield },
  { name: "CrÃ©mation", href: "/cremation", icon: Flame },
  { name: "Catalogue Prix", href: "/catalogue", icon: BookOpen },
    { name: "ComptabilitÃ©", href: "/comptabilite", icon: Calculator },
  ];
        ...
        <div className="p-4 border-t border-sidebar-border/50">
          <div className="flex items-center gap-3 px-3 py-2">
            ...
            <div className="flex-1 overflow-hidden">
              <p className="text-sm font-medium text-sidebar-foreground truncate">Dr. {user?.firstName || "VÃÂÃÂ©tÃÂÃÂ©rinaire"}</p>
        ...
        {currentPath !== "/salle-attente" && (
          <footer className="border-t bg-card/50 py-3 px-4 md:px-6">
            <div className="max-w-6xl mx-auto flex flex-wrap items-center justify-between gap-2 text-xs text-muted-foreground">
              <span>ÃÂÃÂ© {new Date().getFullYear()} VÃÂÃÂ©toAI</span>
              <div className="flex items-center gap-3">
                <a href="/legal" className="hover:text-foreground hover:underline">Mentions lÃÂÃÂ©gales</a>
                <span>ÃÂÃÂ·</span>
                <a href="/confidentialite" className="hover:text-foreground hover:underline">ConfidentialitÃÂÃÂ©</a>
              </div>
            </div>
          </footer>
        )}

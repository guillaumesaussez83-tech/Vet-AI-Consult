...
export default function ImpayesPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!r.ok) throw new Error("Erreur chargement impayÃ©s");
      return r.json();
    },
  });
  ...
  const relanceMutation = useMutation({
    ...
    onSuccess: () => {
      toast({ title: "Relance enregistrÃ©e", description: "La relance a Ã©tÃ© tracÃ©e." });
  ...
  const openRelance = (row: Impaye) => {
    setMessage(
      `Bonjour ${row.owner_name},\n\nNous vous rappelons que la facture ${row.invoice_number} d'un montant de ${fmt(row.reste_a_payer)} est en attente de rÃ¨glement depuis ${row.jours_retard} jours.\n\nMerci de bien vouloir rÃ©gulariser cette situation dans les plus brefs dÃ©lais.\n\nCordialement,\nL'Ã©quipe VÃ©toAI`
    );
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            {impayes.length} facture{impayes.length > 1 ? "s" : ""} â total {fmt(totalImpayes)}
          </p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-sm">
            {filterBucket ? `ImpayÃ©s ${BUCKET_LABELS[filterBucket]}` : "Tous les impayÃ©s"}
            <span className="ml-2 text-muted-foreground font-normal">({filtered.length})</span>
          </CardTitle>
        </CardHeader>
        <CardContent className="p-0">
          {isLoading ? (
            <div className="text-center py-12 text-muted-foreground">Chargementâ¦</div>
          ) : filtered.length === 0 ? (
            <div className="text-center py-12 flex flex-col items-center gap-2 text-muted-foreground">
              <CheckCircle2 className="h-8 w-8 text-green-400" />
              <span>Aucun impayÃ©{filterBucket ? ` dans cette tranche` : ""} ð</span>
            </div>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead className="border-b bg-muted/40">
                  <tr>
                    ...
                    <th className="text-right p-3 font-medium">Reste Ã  payer</th>
                    <th className="text-center p-3 font-medium">AnciennetÃ©</th>
                    ...
                    <tr key={row.id} className="border-b hover:bg-muted/20 transition-colors">
                      ...
                      <td className="p-3 text-right font-bold text-red-600">
                        ...
                        {Number(row.total_paid) > 0 && (
                          <div className="text-xs text-muted-foreground font-normal">
                            {fmt(row.total_paid)} rÃ©glÃ© / {fmt(row.total_ttc)}
                          </div>
                        )}
                      </td>
                      ...
                      <td className="p-3 text-center">
                        {row.nb_relances > 0 ? (
                          <div className="flex flex-col items-center gap-0.5">
                            <Badge variant="secondary" className="text-xs">{row.nb_relances}Ã</Badge>
                      ...
                      <td className="p-3 text-center">
                        <Button
                          ...
                          title={!row.owner_email ? "Email client non renseignÃ©" : "Envoyer une relance"}
                        >
        ...
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              Relance â {relanceDialog?.invoice_number}
            </DialogTitle>
          </DialogHeader>
          {relanceDialog && (
            <div className="space-y-4">
              <div className="bg-muted/40 rounded-lg p-3 text-sm space-y-1">
                ...
                <div><span className="text-muted-foreground">Reste Ã  payer :</span> <strong className="text-red-600">{fmt(relanceDialog.reste_a_payer)}</strong></div>
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-muted-foreground">
                  Ce message sera tracÃ© dans l'historique. L'envoi email doit Ãªtre fait manuellement depuis votre messagerie.
                </p>
              </div>
              ...
              {relanceDialog.nb_relances > 0 && (
                <div className="flex items-center gap-2 text-xs text-muted-foreground bg-yellow-50 border border-yellow-200 rounded p-2">
                  ...
                  <span>
                    {relanceDialog.nb_relances} relance(s) dÃ©jÃ  effectuÃ©e(s),
                    derniÃ¨re le {new Date(relanceDialog.derniere_relance!).toLocaleDateString("fr-FR")}
                  </span>
                </div>
              )}
            </div>
          ...
          <DialogFooter>
            ...
            >
              ...
              {relanceMutation.isPending ? "Enregistrementâ¦" : "Enregistrer la relance"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>

...
// Health endpoints — pas d'auth, pas de rate limiting (Railway health checks)
app.use(healthRouter);
app.use("/api", healthRouter); // /api/health sans auth — Railway healthcheck (fix: avant requireClinicId)

app.use("/api", apiLimiter);
// Rate limiting spécifique IA + génération Factur-X : 10 req/min
app.use("/api/ai", aiLimiter);
app.use("/api/facturx", aiLimiter);

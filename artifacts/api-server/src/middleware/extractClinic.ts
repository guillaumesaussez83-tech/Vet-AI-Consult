...
/**
 * extractClinic() — factory middleware
 * Extrait clinicId depuis les JWT claims Clerk et l'attache à req.clinicId
 * Usage: router.use(extractClinic())
 */
export function extractClinic() {
  return function (req: Request, res: Response, next: NextFunction): void {
    ...
    if (typeof clinicId !== "string" || clinicId.trim() === "") {
      res.status(403).json({ error: "FORBIDDEN", message: "No clinic associated with this account. Contact your administrator." });
      return;
    }

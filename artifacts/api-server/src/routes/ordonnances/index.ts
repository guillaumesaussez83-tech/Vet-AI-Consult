...
router.post("/", async (req, res) => {
  try {
    const { consultationId, patientId, veterinaire, contenu, genereIA, instructionsClient } = req.body;
...
router.post("/ia/generer", async (req, res) => {
  try {
    ...
    const ageStr = dateNaissance
      ? `${Math.floor((Date.now() - new Date(dateNaissance).getTime()) / (365.25 * 24 * 3600 * 1000))} ans`
      : "âge inconnu";
    ...
    const medicaments = actes
      .filter(a => a.categorie === "medicament" || a.categorie === "Médicaments" || a.code?.startsWith("MED"))
      .map(a => `- ${a.nom} (x${a.quantite})${a.description ? ` : ${a.description}` : ""}`)
...
2. Génère les instructions simplifiées pour le propriétaire (section INSTRUCTIONS_CLIENT) : language clair, sans jargon médical
3. Sois précis, professionnel et adapté à l'espèce animale

Réponds en JSON strict (sans markdown) :
{
  "contenu": "texte complet de l'ordonnance médicale",
  "instructionsClient": "texte simplifié pour le propriétaire"
}`;

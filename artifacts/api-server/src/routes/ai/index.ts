...
router.post("/diagnostic", async (req, res) => {
  try {
    ...
    req.log.error(err);
    return res.status(500).json({ error: "Erreur lors de la gÃ©nÃ©ration du diagnostic IA" });
  }
});
...
router.post("/diagnostic-enrichi", async (req, res) => {
  try {
    ...
    if (!anamnese || !examenClinique) {
      return res.status(400).json({ error: "AnamnÃ¨se et examen clinique requis" });
    }
...
router.post("/reformuler-anamnese", async (req, res) => {
  try {
    ...
    req.log.error(err);
    return res.status(500).json({ error: "Erreur lors de la reformulation de l'anamnÃ¨se" });
  }
});
...
router.post("/resume-client", async (req, res) => {
  try {
    ...
    req.log.error(err);
    return res.status(500).json({ error: "Erreur lors de la gÃ©nÃ©ration du rÃ©sumÃ© client" });
  }
});
...
router.post("/generer-facture-voix", pdfLimiter, async (req, res) => {
  try {
    ...
    req.log.error(err);
    return res.status(500).json({ error: "Erreur lors de la gÃ©nÃ©ration de la facture vocale" });
  }
});
...
router.post("/certificat", pdfLimiter, async (req, res) => {
  try {
    ...
    const templates: Record<string, string> = {
      bonne_sante: "Certificat de bonne santÃ© pour voyage UE (Ã©tat gÃ©nÃ©ral, vaccins, puce, date examen)",
      cession: "Certificat de cession pour vente (Ã©tat de santÃ©, vaccins, vermifugations, absence de pathologie connue)",
      aptitude: "Certificat d'aptitude pour concours ou Ã©levage (examen complet systÃ¨mes, locomoteur, cardiaque, respiratoire)",
      soins: "Attestation de soins pour assurance (liste actes, diagnostic, pronostic, durÃ©e traitement)",
      ordonnance: "Ordonnance sÃ©curisÃ©e (numÃ©ro vÃ©tÃ©rinaire, date, posologie dÃ©taillÃ©e, durÃ©e traitement, mentions lÃ©gales)",
    };
    ...
    const prompt = `Tu es vÃ©tÃ©rinaire praticien en France. GÃ©nÃ¨re un ${templateDesc} officiel et professionnel.
...
${consultations?.length ? `CONSULTATIONS RÃCENTES :\n${JSON.stringify(consultations.slice(0, 3), null, 2)}` : ""}
${actes?.length ? `ACTES RÃALISÃS :\n${JSON.stringify(actes, null, 2)}` : ""}

VÃSÃRINAIRE SIGNATAIRE : ${veterinaire || "Dr. VÃ©tÃ©rinaire"}
CLINIQUE : ${ci.nom || clinique || "Clinique vÃ©tÃ©rinaire"}
${adresseComplete ? `ADRESSE : ${adresseComplete}` : ""}
${ci.telephone ? `TÃLÃPHONE : ${ci.telephone}` : ""}
${ci.email ? `EMAIL : ${ci.email}` : ""}
${ci.numeroOrdre ? `NÂ° ORDRE : ${ci.numeroOrdre}` : ""}
${ci.siret ? `SIRET : ${ci.siret}` : ""}
DATE : ${new Date().toLocaleDateString("fr-FR")}

INSTRUCTIONS :
- GÃ©nÃ¨re le certificat complet, professionnel, conforme aux exigences lÃ©gales franÃ§aises.
- Utilise les coordonnÃ©es exactes fournies ci-dessus â n'utilise JAMAIS de placeholders comme [Adresse du cabinet], [XXXX], [email@example.com], etc.
- Inclus toutes les mentions obligatoires.
- Utilise un format structurÃ© avec en-tÃªte (coordonnÃ©es clinique + vÃ©tÃ©rinaire), corps du document et signature.

RÃ©ponds UNIQUEMENT avec le texte du certificat, prÃªt Ã  imprimer.`;
    ...
    if (content.type !== "text") throw new Error("RÃ©ponse inattendue");
    return res.json({ certificat: content.text.trim() });
  } catch (err) {
    req.log.error(err);
    return res.status(500).json({ error: "Erreur lors de la gÃ©nÃ©ration du certificat" });
  }
});
...
router.post("/carnet-vaccinations", async (req, res) => {
  try {
    ...
    const prompt = `Tu es vÃ©tÃ©rinaire. GÃ©nÃ¨re un rÃ©sumÃ© du carnet de santÃ© vaccinal pour ce patient animal.

ANIMAL : ${JSON.stringify(patient)}
PROPRIÃTAIRE : ${JSON.stringify(owner)}
VACCINATIONS : ${JSON.stringify(vaccinations ?? [])}
DATE D'AUJOURD'HUI : ${new Date().toLocaleDateString("fr-FR")}

GÃ©nÃ¨re un bilan vaccinal complet et professionnel qui explique :
1. Les vaccins rÃ©alisÃ©s et leur date
2. Les rappels Ã  venir (prochains 6 mois)
3. Les vaccins en retard s'il y en a
4. Les recommandations vaccinales pour l'espÃ¨ce
5. Un rÃ©sumÃ© de protection actuelle

RÃ©ponds en franÃ§ais, de faÃ§on claire et lisible pour le propriÃ©taire.`;
    const message = await (await import("@workspace/integrations-anthropic-ai")).anthropic.messages.create({
      model: "claude-sonnet-4-6",
      max_tokens: 2048,
      messages: [{ role: "user", content: prompt }],
    ...
    req.log.error(err);
    return res.status(500).json({ error: "Erreur lors de la gÃ©nÃ©ration du carnet" });
  }
});
...
router.post("/dictee-ordonnance", async (req, res) => {
  try {
    ...
    if (!transcription || typeof transcription !== "string" || !transcription.trim()) {
      return res.status(400).json({ error: "Le texte de la dictÃ©e est requis" });
    }
    const { anthropic } = await import("@workspace/integrations-anthropic-ai");
    const message = await anthropic.messages.create({
      model: "claude-sonnet-4-6",
      max_tokens: 2048,
      system: `Tu es un vÃ©tÃ©rinaire praticien expert en France. Tu gÃ©nÃ¨res des ordonnances vÃ©tÃ©rinaires prÃ©cises et mÃ©dicalement justifiÃ©es Ã  partir de dictÃ©es vocales.

RÃGLES STRICTES :
1. Ne prescris JAMAIS d'antibiotiques (amoxicilline, amoxiclavulanate, cÃ©fovecine, mÃ©tronidazole, etc.) sans indication infectieuse confirmÃ©e ou trÃ¨s probable.
2. Calcule toujours les doses en mg/kg si le poids est mentionnÃ©.
3. Prescris UNIQUEMENT ce qui est mÃ©dicalement justifiÃ©.
4. AINS : prÃ©cise la durÃ©e maximale (5-7j chien, 3-5j chat).
5. CorticoÃ¯des : dÃ©croissance progressive.
6. StupÃ©fiants : mention "STUPÃFIANT â Ordonnance sÃ©curisÃ©e obligatoire".
7. quantite_a_delivrer doit Ãªtre un entier positif.
8. ANTI-DOUBLON STRICT : Si un mÃªme mÃ©dicament est mentionnÃ© plusieurs fois dans la dictÃ©e, ne l'inclure QU'UNE SEULE FOIS.
Inclus le champ estDelivre (boolean).`,
      messages: [{
        role: "user",
        content: `Extrait de cette dictÃ©e les prescriptions mÃ©dicamenteuses sous forme JSON structurÃ© :
[{
...
Texte de la dictÃ©e : "${transcription}"`
      }],
    });
    ...
    const normaliserNom = (nom: string): string => (nom ?? "").toLowerCase().normalize("NFD").replace(/[Ì-Í¯]/g, "").replace(/[^a-z0-9]/g, " ").trim().split(/\s+/).slice(0, 3).join(" ");
  ...
  } catch (err) { req.log.error(err); return res.status(500).json({ error: "Erreur lors de la dictÃ©e ordonnance" }); }
});
...
router.post("/confirmer-dictee-ordonnance", async (req, res) => {
  try {
    ...
    const cleanField = (v: any) => { if (v == null) return ""; const s = String(v).trim(); if (!s || s === "â" || s === "-" || /^null$/i.test(s)) return ""; return s; };
    const contenu = prescriptions.map((p: any) => { const delivre = p.estDelivre === false ? " [Ã acheter en pharmacie]" : " [DÃ©livrÃ© en clinique]"; return [p.nom_medicament + delivre, cleanField(p.dose) && `Dose : ${cleanField(p.dose)}`, cleanField(p.voie_administration) && `Voie : ${cleanField(p.voie_administration)}`, cleanField(p.frequence) && `FrÃ©quence : ${cleanField(p.frequence)}`, cleanField(p.duree) && `DurÃ©e : ${cleanField(p.duree)}`].filter(Boolean).join(" â "); }).join("\n");

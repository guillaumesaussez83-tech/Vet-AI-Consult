...
// v1 — Diagnostic différentiel standard
export async function diagnosticDifferentiel(params: DiagnosticParams): Promise<DiagnosticResult> {
  const { espece, race, age, poids, sexe, sterilise, anamnese, examenClinique, examensComplementaires, antecedents, allergies } = params;
...
// v1 — Diagnostic enrichi avec pièces jointes
export async function diagnosticEnrichi(
  params: DiagnosticParams & { objectPaths?: string[] },
  storage: ObjectStorageService
): Promise<DiagnosticResult> {
  const { espece, race, age, poids, sexe, sterilise, anamnese, examenClinique, examensComplementaires, antecedents, allergies, objectPaths } = params;
